...
While taking my second course in the CS department, I fell in love with functional programming.
After finishing that class, I left my janitor job to work as a TA and share my passion for the subject with others.
...
However, modern techniques for reusable code sometimes caused the compiler to loop infinitely when checking useful programs.
For example, a simple function for comparing two binary trees would cause an out-of-memory error\textemdash the compiler could not understand a simple circular definition.
